...
/*   Updated: 2025/01/14 17:18:09 by ynzue-es         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int ind_element, int ind_target)
{
	...
	while (stack_b->arr[stack_b->top] != element
		&& stack_a->arr[stack_a->top] != target)
	{
		if (ind_element >= (stack_b->top / 2) && ind_target >= (stack_a->top
				/ 2))
			rotate_all(stack_a, stack_b);
		else if (ind_element < (stack_b->top / 2) && ind_target < (stack_a->top
				/ 2))
